...
static void	*handleDatagrams(void *parm)
{
	...
	writeErrmsgMemos();
	writeMemo("[i] udpmarsdelaycli receiver thread has ended.");
...
int	main(int argc, char *argv[])
{
	...
	{
		closesocket(rtp.ductSocket);
		putSysErrmsg("udpmarsdelaycli can't create receiver thread", NULL);
		return -1;
	}
...

	{
		...
		isprintf(txt, sizeof(txt),
			"[i] udpmarsdelaycli is running, spec=[%s:%d], Mars delay = %.1f sec, link loss = %.1f%% (timed Mars processing).", 
	...
	destroyTimedBundleQueue(&timedQueue);
	writeErrmsgMemos();
	writeMemo("[i] udpmarsdelaycli has ended.");
	ionDetach();
	return 0;
}