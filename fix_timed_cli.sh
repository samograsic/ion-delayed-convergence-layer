...
# Fix Mars and Moon CLI files by copying the timed processing structure
# from the working preset CLI and adapting delay functions

echo "Applying timed processing fix to Mars and Moon CLI programs..."

for variant in mars moon; do
    echo "Fixing ${variant} CLI with timed processing..."
    ...
    if [ "$variant" = "mars" ]; then
        ...
        sed -i 's/Preset Delay/Mars Delay/g' udp${variant}delaycli_temp.c
        sed -i 's/timed processing/timed Mars processing/g' udp${variant}delaycli_temp.c
...
\treturn distance / SPEED_OF_LIGHT;\
        ...
        sed -i '/\/\* Preset delay in seconds/,/^#endif$/d' udp${variant}delaycli_temp.c
        ...
        sed -i 's/Preset Delay/Moon Delay/g' udp${variant}delaycli_temp.c
        sed -i 's/timed processing/timed Moon processing/g' udp${variant}delaycli_temp.c
        
        # Add Moon-specific constants
...
\treturn distance / SPEED_OF_LIGHT;\
    ...
    echo "Fixed ${variant} CLI with timed processing"
done

echo "All CLI files now use timed processing!"
echo "Building updated programs..."

make udpmarsdelaycli udpmoondelaycli

echo "Timed processing fix complete!"